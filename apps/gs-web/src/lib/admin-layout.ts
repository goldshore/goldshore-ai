...
function applyPreference(pref: LayoutPreference) {
  ...
  for (const [index, el] of modules().entries()) {
...
function snapshot(): LayoutPreference {
  return {
    ...
    modules:modules().map((el,index) => {
      ...
      return { id:moduleId(el,index), order:Number(getComputedStyle(el).order)||index, width:Math.round(rect.width), height:Math.round(rect.height), colSpan, hidden:el.hidden };
    })
  };
}
...
function installDrag() {
  modules().forEach((el,index) => {
    ...
    el.addEventListener('pointerdown', event => {
      ...
      dragging=el; el.dataset.dragging='true'; el.setPointerCapture(event.pointerId);
    });
    el.addEventListener('pointermove', event => {
      ...
      if(!under || under===el || under.parentElement!==el.parentElement) return;
      const parent=el.parentElement!; const children=[...parent.querySelectorAll<HTMLElement>('.gs-module[data-module-id]')];
      ...
      children.forEach((child,i)=>child.style.order=String(i));
    });
    const finish=(event:PointerEvent)=>{if(dragging!==el)return;dragging=null;delete el.dataset.dragging;try{el.releasePointerCapture(event.pointerId);}catch{}queuePersist(250);};
...
function installResize() {
  ...
  let primed=false;
  resizeObserver=new ResizeObserver(entries => { if(!primed)return; if(entries.some(entry => (entry.target as HTMLElement).dataset.resizable==='true')) queuePersist(700); });
...
export async function initializeAdminLayout() {
  ...
  installDensityControl(); installDrag(); installResize();
}
