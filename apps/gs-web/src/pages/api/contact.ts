...
const handleSpam = async (
  ...
  if (env?.DB) {
    await logSubmissionStatus(env.DB, submission.id, submission.formType, 'blocked_spam', 'Spam submission blocked.');
  }
...
const persistSubmission = async (
  ...
  if (env?.DB) {
    storageTasks.push(storeInD1(env.DB, normalizedSubmission, autoResponder));
  }
