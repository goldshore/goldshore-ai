...
const devEnv: ApiEnv & { DEV_AUTH_BYPASS: string; ENV: string } = {
  ...
  PLATFORM_DB: new MemoryDB() as unknown as D1Database,
  GS_ASSETS: new MemoryAssetBucket() as unknown as R2Bucket,
  AI: {} as Ai,
  JWT_SECRET: process.env.JWT_SECRET ?? 'local-dev-jwt-secret',
  STRIPE_API_KEY: process.env.STRIPE_API_KEY ?? 'local-dev-stripe-key',
  SENDGRID_API_KEY: process.env.SENDGRID_API_KEY ?? 'local-dev-sendgrid-key',
  ACCESS_CLIENT_SECRET: process.env.ACCESS_CLIENT_SECRET ?? 'local-dev-access-secret',
