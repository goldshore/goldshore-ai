...
3. Add permissions: **Access: Service Tokens (Edit)** + **Access: Apps and Policies (Edit)**
4. Scope to your account and `goldshore.ai` zone
5. Copy the token value (shown once)
6. Add to GitHub: repo → Settings → Secrets → Actions → New secret
...
|------------|-------------|--------|---------------|
| `GOOGLE_OAUTH_CLIENT_ID` | Cloudflare Worker secrets + GH Actions | ❌ | [console.cloud.google.com → APIs & Services → Credentials](https://console.cloud.google.com/apis/credentials) |
| `GOOGLE_OAUTH_CLIENT_SECRET` | Same | ❌ | Same — OAuth 2.0 Client ID download |
...
> **Note:** `gs-api` Worker reads `GOOGLE_OAUTH_CLIENT_ID` / `GOOGLE_OAUTH_CLIENT_SECRET`.
> Setting `GOOGLE_CLIENT_ID` / `GOOGLE_CLIENT_SECRET` instead will not work — the Worker
> will return 503 on OAuth routes. Use the `GOOGLE_OAUTH_*` names.
...
4. Add authorized redirect URIs:
   - `https://goldshore.ai/auth/google/callback`
   - `https://admin.goldshore.ai/auth/callback`
   - `https://api.goldshore.ai/goldclaw/oauth/google/callback`
5. Download the JSON → extract `client_id` and `client_secret`
6. Store as `GOOGLE_OAUTH_CLIENT_ID` and `GOOGLE_OAUTH_CLIENT_SECRET`
...
4. OAuth flow uses `GOOGLE_OAUTH_CLIENT_ID` + `GOOGLE_OAUTH_CLIENT_SECRET`
...
| AdSense meta tag in `<head>` | ✅ Added to all layouts | `GoldShoreShell.astro`, `WebLayout.astro`, `index.astro` |
...
```
[⚠️] 1. Run setup-cf-agent-access workflow → rotates CF token, creates Access apps, sets secrets
[⚠️] 2. Renew CLOUDFLARE_API_TOKEN in goldshore-gateway              → unblocks PR #213 CI
[⚠️] 3. Add Access permissions to CLOUDFLARE_API_TOKEN before running step 1
[ ] 4. Create Google OAuth credentials → use GOOGLE_OAUTH_CLIENT_ID / GOOGLE_OAUTH_CLIENT_SECRET
[ ] 5. Add all three redirect URIs to Google OAuth config (incl. api.goldshore.ai callback)
[ ] 6. Create Google Chat webhook + add GOOGLE_CHAT_WEBHOOK secret  → enables CI failure alerts
[ ] 7. Enable Google Ads + AdSense APIs in GCP                      → monetization
[ ] 8. Add HP laptop SSH key to GitHub                              → local dev on laptop
```
